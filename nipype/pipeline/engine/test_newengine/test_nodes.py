...
def test_singlenode_1a(inputs_dict, expected_order, expected_output):
    """testing if inputs can be successfully changed after initialization"""
    nn = Node(inputs={"a": np.array([30, 40])}, mapper="a", interface=fun1_interf,
               name="single_node_1")
    nn.inputs = inputs_dict
    nn.run()
    state = namedtuple("state", expected_order)
